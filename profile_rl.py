...
def run_profiling(agent_type, mode, total_steps=50000):
    ...
    for step in range(total_steps):
        ...
        if dact is None: dact = np.array([], dtype=np.int64)
        if cact is None: cact = np.array([], dtype=np.float32)
        if dlp is None: dlp = np.array([], dtype=np.float32)
        if clp is None: clp = np.array([], dtype=np.float32)
        ...
        rv = {
            ...
            "discrete_log_probs": [dlp.flatten() if dlp.size > 0 else np.zeros(n_discrete_heads, dtype=np.float32)],
            "continuous_log_probs": [clp.flatten() if clp.size > 0 else np.zeros(continuous_action_dim, dtype=np.float32)],
            "discrete_actions": [dact.flatten() if dact.size > 0 else np.zeros(n_discrete_heads, dtype=np.int64)],
            "continuous_actions": [cact.flatten() if cact.size > 0 else np.zeros(continuous_action_dim, dtype=np.float32)],
        }
        ...
        if (step + 1) % update_every == 0 and (step + 1) >= batch_size:
            start_time = time.perf_counter()
            if agent_type == "PPO":
                mb = buffer.sample_transitions(idx=np.arange(0, batch_size), as_torch=True, device=device)
                if is_continuous:
    ...
    print(f"Total time taking actions: {total_action_time:.4f}s (Avg: {avg_action_time*1000:.4f}ms)")
    print(f"Total time updating: {total_update_time:.4f}s (Avg: {avg_update_time*1000:.4f}ms)")
...
if __name__ == "__main__":
    ...
    for agent, mode in experiments:
        try:
            res = run_profiling(agent, mode, total_steps=args.steps)
            results.append(res)
    ...
    print("\n" + "="*80)
    print(f"{'Agent':<10} {'Mode':<12} {'Action Total':<15} {'Action Avg(ms)':<15} {'Update Total':<15} {'Update Avg(ms)':<15}")
    print("-"*80)
    for r in results:
        print(f"{r['agent']:<10} {r['mode']:<12} {r['total_action_time']:<15.4f} {r['avg_action_time']*1000:<15.4f} {r['total_update_time']:<15.4f} {r['avg_update_time']*1000:<15.4f}")
    print("="*80)
